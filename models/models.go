...
const CREATE_CONTRACT = 0
const NORMAL_TRANSACTION = 1

const JOIN_OPCODE = 0
const REMOVE_OPCODE = 1
const UPGRADE_OPCODE = 2
const DOWNGRADE_OPCODE = 3
const RAFT = 1
const POW = 2
const POA = 3
const PBFT = 4
...
type MixTransaction struct {
	...
	Status          string      `json:"status"`
	ContractAddress string      `json:"contractAddress"`
	Sipc            string      `json:"sipc" gencodec:"required"`
}
...
type ContractConfig struct {
	AbiData      []byte `json:"abiData"` // 合约abi
	ContractCode string `json:"contractCode"`
}

type ClientPara struct {
	RpcClient *rpc.Client       `json:"rpcClient"` // rpc client
	Client    *ethclient.Client `json:"client"`    // eth client
}

// tx签名
type SignTxPara struct {
	SignPrikeyFile string `json:"signPrikeyFile"` // 签名私钥文件路径 带UTC的那种
	PasswdFile     string `json:"passwdFile"`     // 签名文件的pin文件路径
}

// 交易状态节点结果

type TxResultStatus struct {
...
type EnodeInfo struct {
	...
	Ifminer      uint   `json:"ifminer"` //0:not miner; 1:miner; 2:always miner
	MinerAddress string `json:"minerAddress"`
}

const DATAMATCHERC1155ABI = `[
    {
        "inputs": [
            {
    ...
    },
    {
        "inputs": [
            ...
            {
                "internalType": "uint256[]",
                "name": "tokenTotalSupplies",
                "type": "uint256[]"
            },
            {
                "internalType": "address[]",
                "name": "canBurnAddresses",
                "type": "address[]"
            }
        ],
        "name": "mintBatch",
